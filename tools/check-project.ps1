...
foreach ($duplicate in $duplicateIds) {
  Add-Failure "Duplicate id: $($duplicate.Name) appears $($duplicate.Count)x"
}

$htmlIdSet = $ids | Sort-Object -Unique
...
foreach ($id in $jsIdRefs) {
  if ($htmlIdSet -notcontains $id) {
    Add-Failure "JS references missing id: $id"
  }
}

$localRefs = [regex]::Matches($html, '(?:href|src)="([^"]+)"') | ForEach-Object {
  $_.Groups[1].Value.Split("?")[0]
} | Where-Object {
  $_ -and $_ -notmatch "^(https?:|mailto:|#)"
} | Sort-Object -Unique

foreach ($ref in $localRefs) {
  if (-not (Test-Path (Join-Path $root $ref))) {
    Add-Failure "Missing local reference: $ref"
  }
}

$formControlIds = [regex]::Matches($html, '<(?:input|select|textarea)[^>]*id="([^"]+)"') | ForEach-Object { $_.Groups[1].Value } | Sort-Object -Unique
$labelFors = [regex]::Matches($html, '<label[^>]*for="([^"]+)"') | ForEach-Object { $_.Groups[1].Value } | Sort-Object -Unique
foreach ($controlId in $formControlIds) {
  if ($labelFors -notcontains $controlId) {
    Add-Failure "Form control without label: $controlId"
  }
}

$debugScanPaths = @($htmlPath) + $jsFiles
...
$numberInputs = [regex]::Matches($html, '<input[^>]+type="number"[^>]*>') | ForEach-Object { $_.Value }
foreach ($input in $numberInputs) {
  $idMatch = [regex]::Match($input, 'id="([^"]+)"')
  $id = if ($idMatch.Success) { $idMatch.Groups[1].Value } else { $input }

  if ($input -notmatch '\smin=') { Add-Failure "Number input missing min: $id" }
  if ($input -notmatch '\smax=') { Add-Failure "Number input missing max: $id" }
  if ($input -notmatch '\sstep=') { Add-Failure "Number input missing step: $id" }
}
