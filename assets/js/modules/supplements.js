(function () {
  ...
  function buildSupplementProtocol(data) {
    ...
    if (needsMaleHormoneSupport) {
      stack.push(createSupplementItem("Спортпіт і тестостерон", "Гормональна навігація", "трибулус, D-аспарагінова кислота і бор не є базою; цинк, D3 і магній працюють переважно при дефіциті; екдистерон не стимулює тестостерон напряму, але може розглядатися як потенційний ергогенік для тренувальної працездатності", "спочатку стабілізувати сон, калораж і тренування на 2-4 тижні", "ранковий тестостерон, SHBG, ЛГ/ФСГ, пролактин, естрадіол, 25(OH)D", "обережний"));
    ...
    if (needsMaleHormoneSupport) {
      ...
      caution.push("Трибулус, D-аспарагінова кислота і бор не варто подавати як гарантовані тест-бустери: для здорових тренованих чоловіків ефект на тестостерон слабкий або нестабільний.");
