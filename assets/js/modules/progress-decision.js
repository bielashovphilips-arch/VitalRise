(function () {
  ...
  function collectBaseSignals(metrics) {
    ...
    if ((metrics.goal === "recomp" || metrics.goal === "cut") && (
      (metrics.weightChange !== null && metrics.weightChange > 0.3) ||
      (metrics.waistChange !== null && metrics.waistChange >= 0.5)
    )) {
      actions.push("Якщо це жіночий цикл, познач ПМС/менструацію: у цей період гормональні зміни можуть тимчасово піднімати воду, вагу і об’єми. Не роби висновок по одному заміру; краще порівнювати однакову фазу циклу.");
    }
  ...
  function buildBiweeklyDecision(metrics) {
    ...
    if (!enteredMetrics.length) {
      ...
      actions.push("Для жінок окремо позначати ПМС/менструацію: через гормональні зміни вода, вага і об’єми можуть тимчасово зростати, тому рекомпозицію краще оцінювати по середніх значеннях і однаковій фазі циклу.");
